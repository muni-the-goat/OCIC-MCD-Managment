...
Internal office report tracker (CMS-style): staff submit **monthly or annual budget reports** and **monthly activity reports** through structured forms with file attachments, flowing through a **draft → submitted → reviewed/rejected** workflow, with **Admin / Manager / Staff** role-based access.

## Status: ✅ Deployed baseline — monthly/annual budget update code complete

All application code is written and `npm run build` + ESLint pass. What was built:

1. **Database migrations** — `supabase/migrations/0001_init.sql` creates the schema, RLS, storage policies, and profile trigger; `0002_rename_approved_to_reviewed.sql` renames the terminal positive status; `0003_budget_monthly_grid.sql` adds the section/item Jan–Dec actual-expense grid; `0004_security_hardening.sql` prevents role injection and makes review decisions transactional; `0005_budget_period.sql` adds independent monthly and annual budget periods
...
4. **Reports** — list w/ filters (type/status/author-for-reviewers), new (type picker → form), edit, detail; budget reports toggle between a separate one-month amount layout and the existing full-year Jan–Dec grid, both with freeform sections and subtotals; monthly activity reports have 4 section textareas; all server actions live in `src/app/(app)/reports/actions.ts`
...
1. **User**: run `supabase/migrations/0005_budget_period.sql` in the existing Supabase project before deploying this update
2. Verify one monthly budget and one annual budget through draft → submit → review
